...
func chunkSerializeTest(t *testing.T) {
	...
	chunkTest(t, chunk, env)
	SyntaxTreeTest(t, exprInfos, env)

	fmt.Printf("==========\n")
}

func chunkTest(t *testing.T, chunk *chk.Chunk, ev env.Environment) {
	// 序列化字节码
	...
	start = time.Now()
	runner := gop.NewGopRunner()
	_ = runner.RunChunk(deserializedChunk, ev)

	testdata.CheckValues(t, ev, ser_formulaBatches)
	elapsed = time.Since(start)
	fmt.Printf("字节码执行完成。耗时: %s\n", elapsed)
}

func SyntaxTreeTest(t *testing.T, exprInfos []*ir.ExprInfo, ev env.Environment) {
	// todo 序列化语法树
	// todo反序列化语法树
